...
def train(continue_epi=0, policy_path="../models/template_policy/{}_template_policy.pth",siamfc_path = "../models/siamfc_pretrained.pth",gpu_id=0):
    ...
    pi = T_Policy(T_N)
    weights_init(pi)
    # pretrained_pi_dict = torch.load(policy_paK
    ...
    for train_step in range(MAX_EPISODES):
        ...
        for frame in range(1, length):
            ...
            img_crop_l, _, _ = crop_image_actor_(np.array(img), siam_box)
            ...
            if np.random.random(1) < var or frame <= 3 or frame % 30 == 0:
                ...
                a_ind = np.argmax(np.abs(deta_pos_[0]))
                if(a_ind == 0):
                ...
                a2 = torch.zeros(7).cuda()
                mq = 0
                for j in range(7):
                    a2[j] = 1
                    q_out = q(imo_l.cuda(), a2)
                    if q_out[0] > mq:
                        a = j
            act_pos[a] = 1
            if(a == 1):
            ...
            img_crop_l_, _, out_flag = crop_image_actor_(np.array(img), pos_)
        ...
        if train_step % 400 == 0:
            torch.save(q_target.state_dict(), '../models/Qnet/'+ str(train_step + continue_epi) + 'Qnet.pth')
